...
```
live.yurii.autocatalog.domain/
  make/, model/, generation/, engine/, transmission/, body/, variant/, shared/
  — Pure Java. Zero Spring, zero JPA. No exceptions.

live.yurii.autocatalog.application/
  make/, model/, generation/, engine/, transmission/, body/, variant/, shared/
  — Plain Java services. No @Service annotation.
  — EntityNotFoundException lives in application/shared/

live.yurii.autocatalog.infrastructure/
  persistence/make/, persistence/model/ ...
  config/ApplicationConfig.java  ← Composition Root, all @Bean definitions here
  — JPA entities, repository adapters, Spring config only here

live.yurii.autocatalog.api/
  make/, model/, generation/, engine/, transmission/, body/, variant/, shared/
  — REST controllers, request/response records, GlobalExceptionHandler
```

...
class GenerationControllerTest {
  ...
  void delete_returns_204() throws Exception {
    ...
    doNothing().when(generationService).deleteById(any(GenerationId.class));

    mockMvc.perform(delete("/api/v1/generations/{id}", id))
      .andExpect(status().isNoContent());

    verify(generationService).deleteById(any(GenerationId.class));
  }

  @Test
  void delete_returns_409_when_bodies_exist() throws Exception {
    doThrow(new IllegalStateException("Cannot delete generation: dependent bodies exist"))
      .when(generationService).deleteById(any(GenerationId.class));

    mockMvc.perform(delete("/api/v1/generations/{id}", UUID.randomUUID()))
      .andExpect(status().isConflict())
      .andExpect(jsonPath("$.detail").value("Cannot delete generation: dependent bodies exist"));
  }

  @Test
  void delete_returns_404_when_not_found() throws Exception {
    doThrow(new EntityNotFoundException("Generation not found"))
      .when(generationService).deleteById(any(GenerationId.class));
  ...
  void rename_returns_200_with_updated_name() throws Exception {
    var modelId = ModelId.generate();
    var gen = Generation.create(modelId, "F30", new YearRange(2012, null));
    when(generationService.rename(any(GenerationId.class), eq("G20"))).thenAnswer(invocation -> {
      gen.rename("G20");
      return gen;
    });
    var body = "{\"modelId\":\"" + modelId.value() + "\",\"name\":\"G20\",\"yearFrom\":2019}";

    mockMvc.perform(patch("/api/v1/generations/{id}/name", gen.id().value())
        .contentType(MediaType.APPLICATION_JSON)
        .content(body))
      .andExpect(status().isOk())
      .andExpect(jsonPath("$.name").value("G20"));

    verify(generationService).rename(any(GenerationId.class), eq("G20"));
  }

  @Test
  void rename_returns_404_when_not_found() throws Exception {
    when(generationService.rename(any(GenerationId.class), any()))
      .thenThrow(new EntityNotFoundException("Generation not found"));
    var body = "{\"modelId\":\"" + UUID.randomUUID() + "\",\"name\":\"G20\",\"yearFrom\":2019}";

    mockMvc.perform(patch("/api/v1/generations/{id}/name", UUID.randomUUID())
        .contentType(MediaType.APPLICATION_JSON)
        .content(body))
      .andExpect(status().isNotFound());
  }
  ...
  void rename_returns_409_when_name_already_exists() throws Exception {
    when(generationService.rename(any(GenerationId.class), any()))
      .thenThrow(new IllegalStateException("Generation already exists: G20"));
    var body = "{\"modelId\":\"" + UUID.randomUUID() + "\",\"name\":\"G20\",\"yearFrom\":2019}";

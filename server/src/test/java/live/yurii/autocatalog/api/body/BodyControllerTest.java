...
class BodyControllerTest {
  ...
  void create_returns_409_when_body_style_already_exists() throws Exception {
    when(bodyService.create(any(), any(), any(int.class), any(int.class), any(int.class), any(Integer.class), any(), any()))
  ...
  @Test
  void delete_returns_409_when_variants_exist() throws Exception {
    var id = UUID.randomUUID();
    doThrow(new IllegalStateException("Cannot delete body: dependent variants exist"))
      .when(bodyService).deleteById(new BodyId(id));
  ...
  @Test
  void updateDimensions_returns_200() throws Exception {
    var generationId = GenerationId.generate();
    var body = Body.create(generationId, BodyStyle.SEDAN, 4700, 1820, 1450, 2850);
    body.setTrunkVolumeLitres(500);
    when(bodyService.updateDimensions(any(BodyId.class), eq(4700), eq(1820), eq(1450), eq(2850), eq(500), eq(null)))
      .thenReturn(body);

    var requestBody = """
      {"generationId":"%s","bodyStyle":"SEDAN",
       "lengthMm":4700,"widthMm":1820,"heightMm":1450,"wheelbaseMm":2850,
       "trunkVolumeLitres":500}
      """.formatted(generationId.value());

    mockMvc.perform(patch("/api/v1/bodies/{id}/dimensions", body.id().value())
        .contentType(MediaType.APPLICATION_JSON)
        .content(requestBody))
      .andExpect(status().isOk())
      .andExpect(jsonPath("$.lengthMm").value(4700))
      .andExpect(jsonPath("$.widthMm").value(1820))
      .andExpect(jsonPath("$.trunkVolumeLitres").value(500));
  }
}

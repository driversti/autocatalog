...
class TransmissionControllerTest {
  ...
  void delete_returns_204() throws Exception {
    ...
    doNothing().when(transmissionService).deleteById(any(TransmissionId.class));

    mockMvc.perform(delete("/api/v1/transmissions/{id}", id))
      .andExpect(status().isNoContent());

    verify(transmissionService).deleteById(any(TransmissionId.class));
  }

  @Test
  void delete_returns_409_when_referenced_by_variant() throws Exception {
    doThrow(new IllegalStateException("Cannot delete transmission: referenced by variant(s)"))
      .when(transmissionService).deleteById(any(TransmissionId.class));

    mockMvc.perform(delete("/api/v1/transmissions/{id}", UUID.randomUUID()))
      .andExpect(status().isConflict())
      .andExpect(jsonPath("$.detail").value("Cannot delete transmission: referenced by variant(s)"));
  }

  @Test
  void delete_returns_404_when_not_found() throws Exception {
    doThrow(new EntityNotFoundException("Transmission not found"))
      .when(transmissionService).deleteById(any(TransmissionId.class));

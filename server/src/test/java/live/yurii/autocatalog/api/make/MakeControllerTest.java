...
class MakeControllerTest {
  ...
  void delete_returns_204() throws Exception {
    ...
    doNothing().when(makeService).deleteById(any(MakeId.class));

    mockMvc.perform(delete("/api/v1/makes/{id}", id))
      .andExpect(status().isNoContent());

    verify(makeService).deleteById(any(MakeId.class));
  }

  @Test
  void delete_returns_409_when_models_exist() throws Exception {
    doThrow(new IllegalStateException("Cannot delete make: dependent models exist"))
      .when(makeService).deleteById(any(MakeId.class));

    mockMvc.perform(delete("/api/v1/makes/{id}", UUID.randomUUID()))
      .andExpect(status().isConflict())
      .andExpect(jsonPath("$.detail").value("Cannot delete make: dependent models exist"));
  }

  @Test
  void delete_returns_404_when_not_found() throws Exception {
    doThrow(new EntityNotFoundException("Make not found"))
      .when(makeService).deleteById(any(MakeId.class));
  ...
  @Test
  void updateCountry_returns_200() throws Exception {
    var make = Make.create("BMW", "Germany");
    when(makeService.updateCountry(any(MakeId.class), eq("Austria"))).thenAnswer(invocation -> {
      make.updateCountry("Austria");
      return make;
    });
    var body = "{\"name\":\"BMW\",\"country\":\"Austria\"}";

    mockMvc.perform(patch("/api/v1/makes/{id}/country", make.id().value())
        .contentType(MediaType.APPLICATION_JSON)
        .content(body))
      .andExpect(status().isOk())
      .andExpect(jsonPath("$.country").value("Austria"));

    verify(makeService).updateCountry(any(MakeId.class), eq("Austria"));
  }
}
